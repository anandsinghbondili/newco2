...
export default function ItemsClient() {
    ...
    const fetchItems = async () => {
        setIsLoading(true);
        try {
            const response = await fetch('https://rcxdev.marketmedium.net:8000/items/?skip=0&limit=10');
    ...
    const handleFormSubmit = async (values: ItemFormValues) => {
        ...
        try {
            ...
            if (!response.ok) throw new Error(`Failed to ${values.id ? 'update' : 'create'} item`);
    ...
    return (
        <>
            <RCXSimplePanel title="Items" className="grid grid-cols-1 gap-4 p-2 h-full">
                <RCXSmartGrid
                    columns={columns}
                    data={items}
                    isLoading={isLoading}
                ...
                <ItemForm
                    ...
                    initialValues={{
                        ...
                        id: formValues && typeof formValues.id === 'number' ? formValues.id : null
                    }}
                />
