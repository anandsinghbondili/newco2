...
const RCXRadio: React.FC<RCXRadioProps> = ({ label, helperText, error, required, options, id, value, onChange, name }) => {
    const autoId = React.useId();
    const groupId = id || autoId;
    return (
        <div className="grid gap-1.5">
            {label && (
                <Label htmlFor={groupId} className={required ? 'after:content-["*"] after:ml-0.5 after:text-destructive' : ''}>
                    {label}
                </Label>
            )}
