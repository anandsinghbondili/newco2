...
export function SmartGrid<T>({
  ...
  const [columnVisibility, setColumnVisibility] =
    React.useState<VisibilityState>({ __select: true });
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 p-2">
        <div className="flex items-center gap-2 min-h-0">
          ...
          {toolbarExtra && (
            <div className="ml-2 flex items-center gap-1">{toolbarExtra}</div>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {onRefresh && (
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <RCXSecButton onClick={onRefresh} className="h-8 w-8 p-0">
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <RCXSecButton onClick={resetAll} className="h-8 w-8 p-0">
                ...
                {hg.headers.map((header) => {
                  ...
                  if (header.column.id === "__select") {
                    return (
                      <TableHead
                        key={header.id}
                        className="sticky left-0 bg-primary-foreground w-20 min-w-[33px] max-w-[33px]"
                      >
                        {flexRender(
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {!isMobile && (
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <RCXNeuButton className="h-4 w-4 p-0">
                      ...
                      {!isMobile && (
                        <div
                          ...
                          className={`absolute right-0 top-0 h-full w-1 bg-border cursor-col-resize select-none touch-none opacity-0 group-hover:opacity-100 ${
                  ...
                  {row.getVisibleCells().map((cell) => {
                    if (cell.column.id === "__select") {
                      return (
                        <TableCell
                          key={cell.id}
                          className="sticky left-0 bg-background w-20 min-w-[33px] max-w-[33px]"
                        >
                          {flexRender(
